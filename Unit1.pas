...
then begin
 ...
 // по ID менеджера ищем его фамилию и имя
 IBQuery2.SQL.Clear;
 ...
 // ПО ID-Заказчика ищем самого Заказчика и выставляем соответствующий флаг
 Form5.Edit1.Text:='';
 IBQuery3.SQL.Clear;
 ...
 // Вычисляем по ID локальную фирму
  IBQuery6.SQL.Clear;
 ...
 Form5.SpeedButton7.Click;  // Вход с нормализацией флагов
 form5.show;
end; 
end;
...
begin
 ...
 if MessageDlg('Вы действительно хотите удалить Заказ № '+IBQuery1ID_SHAPKA.AsString+'?',
 ...
 if (fg=1) and (Label2.Caption='Администратор') then
    begin
       if MessageDlg('Заказ содержит выполненные работы! Точно удалить?',
            mtConfirmation, [mbYes, mbNo], 0) = mrYes
            then fg:=0
            else fg:=1;
    end;
...
begin
Form18.SpeedButton1.Click; // Обновляем строки
form18.showmodal;
end;
...
if  (DBGrid1.DataSource.DataSet['DATA_KRAY'] > now())and(DBGrid1.DataSource.DataSet['DATA_KRAY'] < IncDAY(now(), 3)) then
  begin
     If gdSelected in State Then
              begin
              // Осталось 3 дня до выдачи, строка выделена
          ...
          begin
          // Осталось 3 дня до выдачи, строка не выделена
           Brush.Color := $99CCFF;
           FillRect(Rect);
          end
...
if DBGrid1.DataSource.DataSet['DATA_KRAY'] < now() then
  begin
         ...
         If gdSelected in State Then
              begin
              // Дата выдачи просрочена, строка выделена
          ...
          begin
          // Дата выдачи просрочена, строка не выделена
           Brush.Color := $9966FF;
           FillRect(Rect);
          end
...
begin
 // Устройство готово но еще не выдано

 if DBGrid1.DataSource.DataSet['DATA_KRAY'] <= now() then
  begin
     // Прошел крайний срок, но устройство еще не забрали
       If gdSelected in State Then
              begin
              // строка выделена   70A25F
  ...
  begin
    // Устройство готово, но крайний срок еще не наступил
        If gdSelected in State Then
              begin
              // строка выделена 669999
...
if (checkbox2.Checked=true) then begin
                                   ...
                                   fl_v:=2; // ОПРЕДЕЛЯЕМ НЕ ВЫДАННЫЕ ЗАКАЗЫ
                                   fl2:=2;
                                 end;
...
// Заполняем раздел с параметрами
if (checkbox1.Checked=true)and(checkbox6.Checked=false) then begin
...
begin
if unit1.Form1.Label2.Caption<>'Сервис' then
begin
...
begin
 Form19.Caption:='Поиск по номеру карты';
 Form19.Label1.Caption:='№ карты:';
 Form19.Edit4.Left:=Form19.Label1.Left+form19.Label1.Width+5;
 Form19.Label2.Caption:='2'; // Поиск по карте клиента
 Edit3.Text:='';
 Edit4.Text:='';
 Edit5.Text:='';
 Edit6.Text:='';
...
begin
 Form19.Caption:='Поиск по номеру заказа';
 Form19.Label1.Caption:='№ заказа:';
 Form19.Edit4.Left:=Form19.Label1.Left+form19.Label1.Width+5;
 Form19.Label2.Caption:='1'; // Поиск по карте клиента
 Edit3.Text:='';
 Edit4.Text:='';
 Edit5.Text:='';
 Edit6.Text:='';
...
begin
 Form19.Caption:='Поиск по номеру в 1С';
 Form19.Label1.Caption:='№ в 1С:';
 Form19.Edit4.Left:=Form19.Label1.Left+form19.Label1.Width+5;
 Form19.Label2.Caption:='3'; // Поиск по карте клиента
 Edit3.Text:='';
 Edit4.Text:='';
 Edit5.Text:='';
 Edit6.Text:='';
...
begin
 Form19.Caption:='Поиск по номеру устройства';
 Form19.Label1.Caption:='№ устройства:';
 Form19.Edit4.Left:=Form19.Label1.Left+form19.Label1.Width+5;
 Form19.Label2.Caption:='4'; // Поиск по карте клиента
 Edit3.Text:='';
 Edit4.Text:='';
 Edit5.Text:='';
 Edit6.Text:='';
