...
unit3.DataModule3.IBDatabase1.Connected:=true;
      ...
      While not IBQuery3.Eof do
         begin
           Memo1.Lines.Append(datetostr(now())+','+timetostr(time)+': Запись в TODO №'+inttostr(p)); p:=p+1;
           IBQuery1.SQL.Clear;
           ...
           IBQuery2.SQL.Append('CODE_WORK like :CODE_WORK AND ID_ZAKAZ=:ID_ZAKAZ AND WORK like :WORK');
           ...
           if IBQuery2.Eof then
            ...
            // Делаем апдейт
                obn:=0;
                if IBQuery2CENA.Value<>IBQuery3TODO_COST.Value then obn:=1;

                if IBQuery2CODE_USTR.AsString<>IBQuery1USTR_CODE.AsString then obn:=1;
                if IBQuery2CODE_WORK.AsString<>IBQuery3ID_ITEM.asString then obn:=1;
                if IBQuery2DATA_GOTOVO.AsDateTime<>IBQuery3DAT_GOTOVO.AsDateTime then obn:=1;
                if IBQuery2DATA_SOGLASOVANO.AsDateTime<>IBQuery3DAT_SOGL.AsDateTime then obn:=1;
                if IBQuery2DATA_VIDANO.AsDateTime<>IBQuery1DAT_VIDANO.AsDateTime then obn:=1;
                if IBQuery2DATA_ZAKAZ.AsDateTime<>IBQuery3DATA_GET.AsDateTime then obn:=1;
                if IBQuery2FL_GOTOVO.AsInteger<>IBQuery3FL_GOTOVO.AsInteger then obn:=1;
                if IBQuery2FL_SOGLASOVANO.AsInteger<>IBQuery3FL_SOGLASOVANO.AsInteger then obn:=1;
                if IBQuery2FL_VIDANO.AsInteger<>IBQuery1FL_VIDANO.AsInteger then obn:=1;
                if IBQuery2ID_ZAKAZ.AsInteger<>IBQuery3ID_SHAPKA.AsInteger then obn:=1;
                if IBQuery2KOL_VO.AsInteger<>IBQuery3KOL_VO.AsInteger then obn:=1;
                if IBQuery2SUMMA.Value<>IBQuery3TOTAL_COST.Value then obn:=1;
                if IBQuery2WORK.AsString<>IBQuery3TODO_NAME.AsString then obn:=1;
            ...
            IBQuery5.SQL.Append('WHERE CODE_USTR=:CODE_USTR AND CODE_WORK=:CODE_WORK AND ID_ZAKAZ=:ID_ZAKAZ AND WORK like :WORK AND NAME_ITEM like :NAME_ITEM');
     ...
     unit3.DataModule3.IBTransaction2.CommitRetaining;
     Memo1.Lines.Append(datetostr(now())+','+timetostr(time)+': Заканчиваем проверку');
end;
