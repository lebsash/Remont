...
begin
 if unit1.Form1.Label2.Caption='Сервис' then begin
 end
 else
 begin
...
Decoding(unit1.ROLE_USER,unit1.ID_USER, unit1.NAME_USER, r);
 if r='Сервис' then
   begin
    ...
    N7.Visible:=false;  // Согласовано
    N9.Visible:=true;   // Выполнено
    N10.Visible:=false; // Не выполнено
    N11.Visible:=true;  // Не выполнено
    N12.Visible:=false; // Выдано
    N13.Visible:=false; // Не выдано
    N8.Visible:=false;  // Добавить

    N7.Enabled:=false;  // Согласовано
    N9.Enabled:=true;   // Выполнено
    N10.Enabled:=false; // Не выполнено
    N11.Enabled:=true;  // Не выполнено
    N12.Enabled:=false; // Выдано
    N13.Enabled:=false; // Не выдано
    N8.Enabled:=false;  // Добавить
 ...
 begin
    ...
    if label12.Caption='2' then
          ...
          begin
           N9.Visible:=false;   // Выполнено
           N11.Visible:=true;  // Не выполнено
           N9.Enabled:=true;   // Выполнено
           N11.Enabled:=true;  // Не выполнено

          end;
    N10.Visible:=true; // Не согласовано
    N12.Visible:=true; // Выдано
    N13.Visible:=true; // Не выдано
    N8.Visible:=true;  // Добавить

    N7.Enabled:=true;  // Согласовано
    N10.Enabled:=true; // Не согласовано
    N12.Enabled:=true; // Выдано
    N13.Enabled:=true; // Не выдано
    N8.Enabled:=true;  // Добавить


 end;

 if  r='Администратор' then
 begin
    Edit1.Enabled:=true;
    SpeedButton1.Enabled:=true;
...
begin
ListBox1.Clear;
if checkBox1.Checked then ListBox1.Items.Append('Согласовать цену');
if checkBox2.Checked then ListBox1.Items.Append('Согласовать объем работ');
if checkBox3.Checked then ListBox1.Items.Append('Согласовать сроки исполнения');
if checkBox4.Checked then ListBox1.Items.Append('Звонок по готовности');
end;
...
begin
 ...
 WHile not IBQuery2.Eof do
   ...
   if (r1='Менеджер')or(r1='Администратор') then
      begin
       ComboBox2.Items.Append(IBQuery2FAMILY.AsString+' '+IBQuery2NAME.AsString);
       idmanager[i]:=IBQuery2ID.AsInteger; // Массив идентификаторов текущих менеджеров
       i:=i+1;
        SpeedButton3.Enabled:=true;
        SpeedButton1.Enabled:=True;
...
if (strtoint (label8.Caption)<>0) and(strtoint (label9.Caption)<>0)and(strtoint (label12.Caption)<>0) then
 begin
   form12.Label1.Caption:='1'; // Добавление новой записи
   form12.Edit1.text:='';
   form12.label4.Caption:='';
   form12.label11.Caption:='';
   form12.label12.Caption:='';
       // Ищем ID выбранного счета
       IBQuery5.SQL.Clear;
   ...
   // Сохраняем заказ, если он не сохранен
   if label15.Caption='0' then
     begin
       ...
       if DatetimePicker2.Checked=true then  IBQuery4.ParamByName('DATA_END').AsDate:=Trunc(DateTimePicker2.Date)
                             else IBQuery4.ParamByName('DATA_END').Value:=NUlL;                          // Дата выдачи клиенту
       IBQuery4.ParamByName('DATA_GET').AsDate:=Trunc(DateTimePicker1.Date);  // Получено в ремонт
       IBQuery4.ParamByName('DATA_KRAY').AsDate:=Trunc(DateTimePicker3.Date); // Ожидание окончания ремонта

       IBQuery4.ParamByName('F_CLIENT').AsInteger:=IBQuery5ID.AsInteger; // ID выбранной фирмы
      ...
      if checkBox4.Checked then IBQuery4.ParamByName('F_REZ_C').AsInteger:=1
       ...
       IBQuery4.ParamByName('F_STATUS').AsInteger:=1; // Статус объекта
      ...
      // Удалим последовательность на всякий случай
      IBQuery26.SQL.Clear;
     ...
     begin
     // Если в Label15 не ноль - то там идентификатор шапки
     // Производим UPDATE шапки в таблице
     IBQuery7.SQL.Clear;
     ...
     IBQuery7.ParamByName('ABOUT').AsString:=memo2.Text;
        if DatetimePicker2.Checked=true then IBQuery4.ParamByName('DATA_END').AsDate:=Trunc(DateTimePicker2.Date)
                                        else IBQuery7.ParamByName('DATA_END').Value:=Null;                          // Дата выдачи клиенту
       IBQuery7.ParamByName('DATA_GET').AsDate:=Trunc(DateTimePicker1.Date);  // Получено в ремонт
       IBQuery7.ParamByName('DATA_KRAY').AsDate:=Trunc(DateTimePicker3.Date); // Ожидание окончания ремонта

       IBQuery7.ParamByName('F_CLIENT').AsInteger:=IBQuery5ID.AsInteger; // ID выбранной фирмы
      ...
      if checkBox4.Checked then IBQuery7.ParamByName('F_REZ_C').AsInteger:=1
       ...
       IBQuery7.ParamByName('F_STATUS').AsInteger:=1; // Статус объекта
 ...
 else showmessage ('Необходимо выбрать Заказчика, Исполнителя');
end;
...
// StringGrid1.RowCount:=IBQuery8.RecordCount+3;
 ...
 advStringGrid1.ColCount:=16;
    ...
    advStringGrid1.ColWidths[7]:=0;
     ...
     advStringGrid1.ColWidths[9]:=50;  // ГОТОВО



    advStringGrid1.ColWidths[10]:=50;  // Согласовано
     advStringGrid1.ColWidths[11]:=50; // Выдано
    ...
    advStringGrid1.Cells[9,0]:='Готово';
        advStringGrid1.Cells[10,0]:='Соглас.';
                advStringGrid1.Cells[11,0]:='Выдан';
                advStringGrid1.Cells[13,0]:='КОД';
               advStringGrid1.Cells[15,0]:='Исполнитель';
 IBQuery9.First;
 i:=1;
...
begin
if (r='Сервис') and (advStringGrid1.Cells [14, advStringGrid1.Row]<>inttostr(unit1.ID_USER)) then begin
  end else
  begin
  // РАССТАНОВКА ФЛАГА НЕ СОГЛАСОВАНО
if l1=1 then
 if (label15.Caption<>'0')and(NOT IBQuery8.Eof) then
 begin
   // Приступаем к согласованию

   // Если выбран айтем - согласуем айтем целиком
    if advStringGrid1.Cells [1, advStringGrid1.Row]<>'' then
       begin
       // Showmessage ('Согласуем весь ITEM')
        // Согласуем все работы внутри айтема
         IBQuery22.SQL.Clear;
         ...
         // Ставим время согласования на весь айтем (для айтема)speedbutton7.Click; // Согласуем остатки и обновляем выдачу
          speedbutton7.Click; // Согласуем остатки и обновляем выдачу
         

       end
       else
       begin
        // Showmessage ('Согласуем конкретную задачу');
         IBQuery22.SQL.Clear;
...
if (r='Сервис') and (advStringGrid1.Cells [14, advStringGrid1.Row]<>inttostr(unit1.ID_USER)) then begin
 ...
 unit2.DataModule2.IBTransaction1.CommitRetaining;
 Speedbutton7.Click; // Нормализуем и обновляем выдачу
 end;
end;
...
begin
if MessageDlg('Вы действительно хотите удалить позицию '+advStringGrid1.Cells [1, advStringGrid1.Row]+' из заказа?',
...
// ПО ФЛАГУ ЗАВЕРШЕНИЯ РАБОТ
if  strtoint(advStringGrid1.Cells [10, advStringGrid1.Row])=1 then // Если согласовано - разрешаем окончание работы
 begin
  // При наличии согласования можно рассогласовать
  if l1=1 then
             begin
              N10.Enabled:=true; // Не согласовано
              N10.Visible:=true;
              N5.Visible:=false;
              N5.Enabled:=false; // согласовано
             end
             else
             begin
              N10.Enabled:=false; // Не согласовано
              N10.Visible:=false;
              N5.Visible:=false;
              N5.Enabled:=false; // согласовано
             end;
...
if  (strtoint(advStringGrid1.Cells [9, advStringGrid1.Row])=0) then
                                                          begin
                                                            if (l3=1)or(label12.Caption='2') then begin
                                                                                        N9.Visible:=true; // ГОТОВО
                                                                                        N11.Visible:=false; // Не ГОТОВО
                                                                                        N9.Enabled:=true; // ГОТОВО
                                                                                        N11.Enabled:=false; // Не ГОТОВО


                                                                          end
                                                                          else
                                                                          begin
                                                                                        N9.Visible:=false; // ГОТОВО
                                                                                        N11.Visible:=false; // Не ГОТОВО
                                                                                        N9.Enabled:=false; // ГОТОВО
                                                                                        N11.Enabled:=false; // Не ГОТОВО
                                                                          end;
                                                           // Если не готово - выдать нельзя
                                                            if l2=1 then begin
                                                           ...
                                                           begin
                                                            if (l3=1)or(label12.Caption='2') then begin
                                                                                        N9.Visible:=false; // ГОТОВО
                                                                                        N11.Visible:=true; // Не ГОТОВО
                                                                                        N11.Enabled:=true; // ГОТОВО
                                                                                        N9.Enabled:=false; // Не ГОТОВО

                                                                                     end else
                                                                                       begin
                                                                                        N9.Visible:=false; // ГОТОВО
                                                                                        N11.Visible:=false; // Не ГОТОВО
                                                                                        N11.Enabled:=false; // ГОТОВО
                                                                                        N9.Enabled:=false; // Не ГОТОВО
                                                                                       end;
                                                           // Если готово - то согласовать нельзя, можно выдать
                                                           if l1=1 then begin
                                                                          // Редактировать готовую работу нельзя
                                                                          N2.Visible:=false;
                                                                          N2.Enabled:=false;
                                                                          N3.Visible:=False;
                                                                          N3.Enabled:=false;
              ...
              begin
               // Завершить работу нельзя, отсутствуют согласования
                     N9.Visible:=false; // ГОТОВО
                     N11.Visible:=false; // Не ГОТОВО
                     N9.Enabled:=false; // ГОТОВО
                     N11.Enabled:=false; // Не ГОТОВО
...
if  advStringGrid1.Cells [1, advStringGrid1.Row]='' then
           begin
            // Просто работу нельзя выдать, можно выдать весь айтем
            N12.Visible:=false;
            N13.Visible:=false;
           end;
...
// По текущему идентификатору айтема смотрим, выдано ли клиенту, если клиенту выдали - то мы не можем
// отменить готовность, а, как следствие, отменить согласование всего айтема и каждого элемента в отдельности
 IBQuery24.SQL.Clear;
    ...
    if r='Сервис'  then begin
                   ...
                   if advStringGrid1.Cells [1, advStringGrid1.Row]<>'' then
                                 begin
                                       ...
                                       // Позволяем печатать дефектную ведомость
                                       N14.Visible:=true;
...
begin
 // Начинаем нормальизацию в соответствии с работами
 IBQuery16.SQL.Clear;
 ...
 //IBQuery16.ParamByName('FL_GOTOVO').AsInteger:=0; // Работы не завершены
 //IBQuery16.ParamByName('FL_VIDANO').AsInteger:=0; // Айтем не выдан клиенту
 IBQuery16.Close;
 IBQuery16.Open;
 IBQuery16.First;
 While not IBQuery16.Eof do
  begin
    // По каждому конкретному айтему смотрим список работ
...
// ВСЕ ЛИ ВЫДАНО - ОБНОВЛЕНИЕ В ШАПКЕ
...
//  IBQuery19.ParamByName('FL_GOTOVO').AsInteger:=1;
  ...
  SpeedButton6.Click;   // Обновляем выдачу
...
if unit1.Form1.Label2.Caption='Сервис' then begin
         if strtoint(label15.Caption)<>0 then
          begin
     //Мастеру разрешается исправлять только полe ABOUT
     IBQuery7.SQL.Clear;
...
if (r='Сервис') and (advStringGrid1.Cells [14, advStringGrid1.Row]<>inttostr(unit1.ID_USER)) then begin
 ...
 if (label15.Caption<>'0')and(NOT IBQuery8.Eof) then
 begin
 // Расстановка флага ВЫДАНО КЛИЕНТУ
 IBQuery23.SQL.Clear;
...
if (r='Сервис') and (advStringGrid1.Cells [14, advStringGrid1.Row]<>inttostr(unit1.ID_USER)) then begin
 ...
 if (label15.Caption<>'0')and(NOT IBQuery8.Eof) then
 begin
 // Расстановка флага НЕ ВЫДАНО КЛИЕНТУ
 IBQuery23.SQL.Clear;
 IBQuery23.SQL.Append('UPDATE REM_USTR SET FL_VIDANO=:FL_VIDANO, DAT_VIDANO=:DAT_VIDANO WHERE ID_SHAPKA=:ID_SHAPKA AND ID=:ID');
...
if  advStringGrid1.Cells [1, advStringGrid1.Row]='' then  begin
                                                               if (r='Сервис') and (advStringGrid1.Cells [14, advStringGrid1.Row]<>inttostr(unit1.ID_USER)) then
                                                               begin
                                                               end else N2.Click;
                                                          end
                                                    else N6.click;
