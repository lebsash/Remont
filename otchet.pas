...
begin
 ...
 procentov:=0;
 pl:=0;
 // Начинаем считать по работам на основе шапки IBQuery1
 ...
 advStringGrid1.Cells[1,0]:='Сумма';
 advStringGrid1.Cells[2,0]:='Процент';
 ...
 For i:=1 to col do
  begin
   ...
   advStringGrid1.Cells[0,col+1]:='СУММА';
   ...
   advStringGrid1.Cells[0,col+2]:='РАБОТ';
...
begin
 ...
 begin
 frxreport1.Variables['NV']:=''''+'Начало выборки'+'''';
 ...
 frxreport1.Variables['OV']:=''''+'Окончание выборки'+'''';
 end
 else
 begin
 ...
 if length (Edit1.text)>0 then  frxreport1.Variables['ZAK']:=''''+'Заказчик:'+Edit1.text+''''
                          else  frxreport1.Variables['ZAK']:=''''+''+'''';
 if length (Edit2.text)>0 then  frxreport1.Variables['ISP']:=''''+'Исполнитель:'+Edit2.text+''''
